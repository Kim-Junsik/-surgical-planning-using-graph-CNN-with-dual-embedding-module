...
import numpy as np
from models.readout import readout_function
...
class Direction_GCN(nn.Module):
    ...
    def __init__(self, args, device):
        super(Direction_GCN, self).__init__()
        self.args = args
        ...
        self.mlp = nn.Linear(args.hidden_dim[-2], args.direc_dim)
    def _L2_norm(self, A):
        # A -> V*V
    ...
    def forward(self, x: torch.Tensor, absolute, L2, softmax, sigmoid, clip):
        ...
        for idx in range(len(self.bns)):
            ...
            if self.args.bn:
                x = self.bns[idx](x)
        x1 = self.blocks[-1](x, A, self.Identity)

        x2 = readout_function(x, 'sum')
        if x.shape[0]==1:
            x2=x2.unsqueeze(0) 
        x2 = self.mlp(x2)
        return x1, x2
